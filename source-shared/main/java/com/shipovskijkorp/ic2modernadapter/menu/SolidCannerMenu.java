...
/** Original IC2 Solid Canning Machine menu layout. */
public final class SolidCannerMenu extends AbstractContainerMenu {
    public static final int SLOT_COUNT = AbstractSolidCannerBlockEntity.SOLID_CANNER_SLOT_COUNT;
    public static final int DATA_COUNT = AbstractElectricMachineBlockEntity.DATA_COUNT;

    private final Container container;
    private final ContainerData data;

    public SolidCannerMenu(MenuType<?> type, int containerId, Inventory playerInventory) {
        this(type, containerId, playerInventory, new SimpleContainer(SLOT_COUNT), defaultClientData());
    }

    public SolidCannerMenu(MenuType<?> type, int containerId, Inventory playerInventory, Container container, ContainerData data) {
        ...
        addSlot(new Slot(container, AbstractElectricMachineBlockEntity.SLOT_INPUT, 37, 36) {
        ...
        addSlot(new Slot(container, AbstractSolidCannerBlockEntity.SLOT_CAN_INPUT, 67, 36) {
            @Override public boolean mayPlace(ItemStack stack) { return container.canPlaceItem(AbstractSolidCannerBlockEntity.SLOT_CAN_INPUT, stack); }
        });
        addSlot(new Slot(container, AbstractElectricMachineBlockEntity.SLOT_OUTPUT, 116, 36) {
            @Override public boolean mayPlace(ItemStack stack) { return false; }
        });
        addSlot(new Slot(container, AbstractElectricMachineBlockEntity.SLOT_DISCHARGE, 8, 62) {
    ...
    private static ContainerData defaultClientData() {
        ...
        data.set(AbstractElectricMachineBlockEntity.DATA_CAPACITY, (int) MachineSpec.SOLID_CANNER.capacityEu());
        data.set(AbstractElectricMachineBlockEntity.DATA_MAX_PROGRESS, MachineSpec.SOLID_CANNER.operationTicks());
        data.set(AbstractElectricMachineBlockEntity.DATA_MACHINE, MachineSpec.SOLID_CANNER.ordinal());
        return data;
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (container.canPlaceItem(AbstractSolidCannerBlockEntity.SLOT_CAN_INPUT, source)) {
            if (!moveItemStackTo(source, AbstractSolidCannerBlockEntity.SLOT_CAN_INPUT, AbstractSolidCannerBlockEntity.SLOT_CAN_INPUT + 1, false)) return ItemStack.EMPTY;
