...
public final class ToolBoxMenu extends AbstractContainerMenu {
    ...
    private void load(ItemStack stack) {
        ...
        ContainerHelper.loadAllItems(root, items, registries);
    ...
    private void save() {
        ...
        ContainerHelper.saveAllItems(root, items, registries);
        CustomData customData = toolboxStack.get(DataComponents.CUSTOM_DATA);
        CompoundTag tag = customData == null ? new CompoundTag() : customData.copyTag();
        tag.put(ROOT_TAG, root);
        CustomData.set(DataComponents.CUSTOM_DATA, toolboxStack, tag);
    }
}
